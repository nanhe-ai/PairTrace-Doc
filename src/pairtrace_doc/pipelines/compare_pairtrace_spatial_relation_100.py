...
def run(config_path: Path) -> dict[str, Any]:
    ...
    if runtime["gpu_launch_authorized"] or runtime["final_reserve_read_allowed"]:
        raise ValueError("spatial-relation decision cannot use GPU or final reserve")
    if runtime["paper_evidence"] or config["experiment"]["paper_evidence"]:
        raise ValueError("spatial-relation decision cannot be paper evidence")
    ...
    if _sha256(protocol_path) != config["experiment"]["expected_protocol_sha256"]:
        raise ValueError("spatial-relation decision protocol SHA-256 changed")
    ...
    for name, mode in (("correct", "correct_pair"), ("shuffled", "shuffled_pair")):
        if summaries[name]["pair_mode"] != mode:
            raise ValueError(f"spatial-relation {name} pair mode changed")
        if summaries[name]["final_reserve_read"]:
            raise ValueError(f"spatial-relation {name} opened the final reserve")
        if summaries[name]["protocol_sha256"] != config["experiment"][
            "expected_protocol_sha256"
        ]:
            raise ValueError(f"spatial-relation {name} protocol hash changed")
    metrics = {
