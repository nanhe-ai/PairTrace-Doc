...
def run(config_path: Path, project_root: Path) -> dict[str, Any]:
    ...
    def index(rows: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
        ...
        if len(output) != len(rows) or set(output) != expected_ids:
            raise ValueError("V2.1 result rows do not match frozen source IDs")
        return output
    ...
    for placement_id in sorted(expected_ids):
        ...
        candidate_bound = str(visual_row["candidate_sha256"]) == str(
        ...
        outcomes.append(
            {
                ...
                "accepted_visual_gate": visual_row.get("decision") == "accept",
                "method_id": str(run_row["method_id"]),
                "ocr_rectified_boundary_exact_replacement": bool(
                    audit_row["checks"][
                        "ocr_rectified_boundary_exact_replacement"
                    ]
                ),
                "outside_changed_pixels": int(
                    audit_row["outside_changed_pixels"]
                ),
                "run_and_audit_agree": bool(audit_row["run_and_audit_agree"]),
                "v2_placement_id": placement_id,
                "visual_candidate_hash_bound": candidate_bound,
    ...
    gate_checks = {
        ...
        "all_sources_have_rectified_ocr_replacement": all(
            row["ocr_rectified_boundary_exact_replacement"] for row in outcomes
        ),
        "all_sources_have_zero_outside_change": all(
            row["outside_changed_pixels"] == 0 for row in outcomes
        ),
        "all_visual_reviews_bound_to_candidates": all(
            row["visual_candidate_hash_bound"] for row in outcomes
        ),
        "run_and_audit_reports_agree": bool(run_report["automatic_gate_passed"])
        == bool(audit_report["automatic_gate_passed"])
        and bool(audit_report["run_and_audit_reports_agree"])
        and all(row["run_and_audit_agree"] for row in outcomes),
    }
    ...
    result = {
        "authorization": {
            ...
            "further_cpu_redesign_authorized": False,
        ...
        "cpu_render_route_terminated": not placement_passed,
        ...
        "rows": len(outcomes),
        "status": (
            "v2_1_placement_preflight_passed_hybrid_toy_design_may_begin"
            if placement_passed
            else "v2_1_placement_preflight_failed_cpu_render_route_terminated_hybrid_toy_pilot100_final150_closed"
        ),
        "visual_accepted": sum(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Finalize the V2.1 placement toy")
